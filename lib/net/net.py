...
from lib.utils.parse_config import parse_model_config
from lib.net.module import Upsample
from lib.net.module import YOLOLayer
from lib.net.module import weights_init_normal
...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            # 如果模型的結構為["convolutional", "upsample", "maxpool"]時，直接將x帶入
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                x = module(x)
            # 如果模型的結構為"route"，做完下面處理在帶進去
            elif module_def["type"] == "route":
